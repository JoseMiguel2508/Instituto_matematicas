...
6. **Base de Datos**
   ...
   - Importa la base de datos de tu preferencia (solo necesitas importar una de las dos opciones):
     
     **Opción A (Recomendada para iniciar limpio):**
     Importa el esquema estructural oficial sin datos de prueba:
     ```bash
     mysql -u tu_usuario -p InstitutoMatematicas < esquema_bd.sql
     ```
     
     **Opción B (Para desarrollo y demostraciones):**
     Importa el volcado completo que incluye estructura y todos los datos de prueba masivos (alumnos, pagos, notas, horarios, etc.):
     ```bash
     mysql -u tu_usuario -p InstitutoMatematicas < datos_prueba.sql
     ```
     
   *(Nota: Este proyecto no utiliza migraciones estándar de Laravel, todo el esquema y relaciones se manejan directamente a través de estos archivos SQL)*.
